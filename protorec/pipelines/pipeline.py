...
class CameraPipeline(BasePipeline):
    ...
    def _play(cls, pipeline: Gst.Pipeline) -> bool:
        """Set the pipeline to PLAYING, resetting it to NULL on failure.
        ...
        -------
        bool
            True if the pipeline started, False if it failed
        """
        if pipeline.set_state(Gst.State.PLAYING) != Gst.StateChangeReturn.FAILURE:
            return True

        logger.warning(
            "Could not start %s: %s", pipeline.get_name(), cls._pop_error(pipeline)
        )
